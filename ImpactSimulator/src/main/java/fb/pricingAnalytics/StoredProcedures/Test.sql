USE [ImpactSimulator]
GO
/****** Object:  StoredProcedure [dbo].[MenuitemSelectProc_Test]    Script Date: 11/28/2019 1:58:10 AM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER   PROCEDURE [dbo].[MenuitemSelectProc_Test] @startRowIndex int=0,
@pageSize int=100,
...
FROM [dbo].[IST_Store_Info] [IST_Store_Info] 
where 
...
IST_Product_Tier_Info_Var
AS
(
SELECT
*
FROM [dbo].[IST_Product_Tier_Info] [IST_Product_Tier_Info] 
where 
and [IST_Product_Tier_Info].BrandId=@BrandId  
and [ IST_Product_Tier_Info.Project_Id=@Project_Id 
and IST_Product_Tier_Info.Scenario_ID =@Scenario_Id 
),
...
FROM IST_Store_Product_Info_Var [IST_Store_Product_Info]
LEFT JOIN IST_Store_Info_Var [IST_Store_Info] ON ([IST_Store_Product_Info].BrandId = [IST_Store_Info].BrandId and
...
) as a LEFT JOIN IST_Product_Tier_Info_Var AS IST_Product_Tier_Info ON 
